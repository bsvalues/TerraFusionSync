...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_complete_gis_export_workflow(cleanup_test_exports):
    ...
    # Submit job
    response = client.post("/plugins/v1/gis_export/run", json=export_data)
    assert response.status_code == 202
    job_id = response.json()["job_id"]
    
    # Check status initially
    response = client.get(f"/plugins/v1/gis_export/status/{job_id}")
    assert response.status_code == 200
    ...
    # Get results
    response = client.get(f"/plugins/v1/gis_export/results/{job_id}")
    assert response.status_code == 200
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_failed_gis_export_job(cleanup_test_exports):
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_cancel_gis_export_job(cleanup_test_exports):
    """Test cancelling a GIS export job."""
    # Create a job
    job_id = await test_create_gis_export_job(cleanup_test_exports)
    
    # Cancel the job
    response = client.post(f"/plugins/v1/gis_export/cancel/{job_id}")
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_list_gis_export_jobs(cleanup_test_exports):
    """Test listing GIS export jobs with various filters."""
    # Create a few test jobs
    job_id1 = await test_create_gis_export_job(cleanup_test_exports)
    ...
    response = client.get("/plugins/v1/gis_export/list")
    assert response.status_code == 200
    data = response.json()
    assert len(data) >= 2  # Should have at least our 2 test jobs
    
    # Test filtering by county_id
    response = client.get(f"/plugins/v1/gis_export/list?county_id={TEST_COUNTY_ID}")
    assert response.status_code == 200
    data = response.json()
    assert len(data) >= 2
    
    # Test filtering by export_format
    response = client.get(f"/plugins/v1/gis_export/list?export_format={TEST_EXPORT_FORMAT}")
    assert response.status_code == 200
    data = response.json()
    ...
    response = client.get("/plugins/v1/gis_export/list?status=RUNNING")
    assert response.status_code == 200
    running_jobs = response.json()
    
    response = client.get("/plugins/v1/gis_export/list?status=COMPLETED")
    assert response.status_code == 200
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_gis_export_plugin_health(cleanup_test_exports):
    """Test the GIS Export plugin health check endpoint."""
    response = client.get("/plugins/v1/gis_export/health")
